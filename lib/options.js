...
function parseThemeArg(theme, assetsPath) {
...
function parseAssetsArg(assets, assetsPath) {
  return (typeof assets === 'string' ? assets.split(',') : []).map(asset => `${assetsPath}/${asset}`);
}
...
function parseOptions(args) {
  ...
  options.themePath = parseThemeArg(args.theme || defaults.theme, args.assetsPath);
  options.scriptPaths = parseAssetsArg(args.scripts, args.assetsPath);
  options.scriptSources = parseAssetsPath(args.scripts);
  options.cssPaths = parseAssetsArg(args.css, args.assetsPath);
