...
const mdImageRE = /!\[([^\]]*)\]\(([^)]+)\)/gi;
const bgImageRE = /data-background-image=["']([^"']+)["']/gi;

function prefixImages(markdown, options) {
  let image;
  const imagePath = options.staticImagePath;
  const awaits = [];
  ...
  while ((image = mdImageRE.exec(markdown))) {
    const imgMarkdown = image[0],
      imgTitle = image[1],
      imgPath = image[2],
      imgStaticPath = path.join(imagePath, imgPath);
    awaits.push(
        markdown = markdown.replace(imgMarkdown, `![${imgTitle}](${imgStaticPath})`)
    );
  }

  while ((image = bgImageRE.exec(markdown))) {
    const bgMarkdown = image[0],
      bgPath = image[1],
      bgStaticPath = path.join(imagePath, bgPath);
    awaits.push(
        markdown = markdown.replace(bgMarkdown, `data-background-image="${bgStaticPath}"`)
    );
  }
...
function embedImages(markdown, options) {
  ...
  while ((image = bgImageRE.exec(markdown))) {
    const bgMarkdown = image[0],
      bgPath = image[1];
    awaits.push(
      imageDataURI.encodeFromFile(path.join(options.basePath, bgPath)).then(dataUri => {
        markdown = markdown.replace(bgMarkdown, `data-background-image="${dataUri}"`);
      })
    );
  }
...
function renderImages(markdown, options) {
  if (options.staticImagePath) {
    return prefixImages(markdown, options);
  }
  else {
...
module.exports = function renderStaticMarkup(options) {
  ...
  const assetsDir = path.join(targetPath, options.assetsPath);
  ...
  const markupAwait = parseSlideAwait
    .then(markdown => renderImages(markdown, options))
